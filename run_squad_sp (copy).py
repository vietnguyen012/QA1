...
def _check_is_max_context(doc_spans, cur_span_index, position):
  ...
  for (span_index, doc_span) in enumerate(doc_spans):
    ...
    score = min(num_left_context, num_right_context) + 0.1 * doc_span.length
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    score_null = 1000000000  # large and positive
...
def main(_):
  ...
  if FLAGS.do_export:
    ...
    predict_input_fn=input_fn_builder(
        input_file='/home/saplab/albert_vi/Data/predict_file1',
        seq_length=FLAGS.max_seq_length,
        is_training=False,
        drop_remainder=False)
    save_hook = tf.train.CheckpointSaverHook("/home/saplab/albert_vi/output", save_secs=1)
    estimator.predict(input_fn=predict_input_fn, hooks=[save_hook])
    estimator.export_saved_model(
        export_dir_base = "/home/saplab/albert_vi/Export",
