...
flags.DEFINE_bool("do_client", False,"make client")
...
flags.DEFINE_string(
    "context", None,
    "context file path of Text summary to feed the AlBERT model ")
...
def read_squad_examples(input_file=None,data=None, is_training=False,input_data=False):
    ...
    for paragraph in entry["paragraphs"]:
      ...
      for qa in paragraph["qas"]:
        ...
        if is_training:
          ...
          if not is_impossible and not (qa['answers'][0]['text'] =="" and qa['answers'][0]['answer_start'] == -1) :
            answer = qa["answers"][0]
...
def _check_is_max_context(doc_spans, cur_span_index, position):
  ...
  for (span_index, doc_span) in enumerate(doc_spans):
    ...
    score = min(num_left_context, num_right_context) + 0.019 * doc_span.length
...
def create_model(albert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  output_weights = tf.get_variable(
      ...
      initializer=tf.truncated_normal_initializer(stddev=0.213))
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    score_null = 1000  # large and positive
...
#     tf.logging.set_verbosity(tf.logging.INFO)
    ...
    return albert_config, run_config, tokenizer

def model_definition(albert_config, run_config):
    train_examples = None
...
#max_steps=num_train_steps
  ...
  if FLAGS.do_export:
    ...
    predict_input_fn=input_fn_builder(
        input_file='/home/vietnguyen/albert_vi/Data/predict_file1',
        seq_length=FLAGS.max_seq_length,
        is_training=False,
        drop_remainder=False)
    save_hook = tf.train.CheckpointSaverHook("/home/vietnguyen/albert_vi/output/", save_secs=1)
    estimator.predict(input_fn=predict_input_fn, hooks=[save_hook])
    estimator.export_saved_model(
        export_dir_base = "'/home/vietnguyen/albert_vi/Export",
  ...
  if FLAGS.do_predict:
    ...
    if (tf.gfile.Exists(FLAGS.predict_feature_file) and tf.gfile.Exists(FLAGS.predict_feature_left_file)): # pylint: disable=unused-argument
      """The `model_fn` for TPUEstimator."""
